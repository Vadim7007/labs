...
namespace Prog3 {
	...
	int Map::HelpfulFunction(int barrel, int occupy) {
		int Cnumber, FBcount, count = 0; //count считает, сколько €чеек соответствует
		for (int Lnumber = 0; Lnumber < UL; Lnumber++) {
			FBcount = 0; //счЄтчик непустых €чеек в строке (free/busy)
			Cnumber = 0;
	...
	Fullness Map::MapState() const {
		Fullness exist = FREE; //отвечает за состо€ние карты
	...
	struct Cell* Map::FreeCells() const {
		...
		int i, count = 0, Cnumber, FBcount; //count - текущий размер массива свободных €чеек
		for (int Lnumber = 0; Lnumber < UL; Lnumber++) {
			FBcount = 0;
			Cnumber = 0;
		...
		if (count == 0)
			mtx[count].type = EMPTY; //дл€ случа€, когда массив пуст
		else
